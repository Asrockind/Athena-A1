...
int main(int argc, char *argv[]) {
    try {
        ...
        cv::VideoCapture cap;
//ASROCKind >>>> open IP cam 
//		if (!(FLAGS_i == "cam" ? cap.open(0) : cap.open(FLAGS_i))) {
        if (!(FLAGS_i == "cam" ? cap.open(0) : cap.open("http://admin:admin@192.168.1.200/GetData.cgi?CH=1"))) {  
//ASROCKind <<<< open IP cam 
        ...
        size_t id = 0;
//ASROCKind >>>> every 2 frames into intel model
        int model_frame = 2;
//ASROCKind <<<< every 2 frames into intel model
        if (FLAGS_fps > 0) {
...
//ASROCKind >>>> every 2 frames into intel model
		      if(framesCounter%model_frame==0){
